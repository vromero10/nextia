...
import { getAllIAPosts, getIAPostBySlug } from '@/lib/mdx'
...
// Next.js usa esto para pre-renderizar todas las páginas de posts de IA
export async function generateStaticParams() {
  const posts = await getAllIAPosts() // Obtiene todos los posts de IA
  return posts.map((p) => ({ slug: p.slug })) // Retorna array de slugs para generar rutas
}

// Componente principal de la página de post individual de IA
export default async function IAPost({ params }: { params: { slug: string } }) {
  // Obtiene el post específico usando el slug de la URL
  const post = await getIAPostBySlug(params.slug)
  if (!post) return null // Si no existe el post, no renderiza nada
          ...
          {meta?.image && (
            <div className="section-panel overflow-hidden mb-6">
              <Image
                ...
                priority // Carga con prioridad alta
              />
            </div>
          )}
